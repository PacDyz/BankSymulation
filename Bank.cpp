...
{
	listOfAccount.insert(std::make_move_iterator(listOfAccount.begin()),
						std::make_move_iterator(listOfAccount.end())); // c++17 this->listOfAccount.merge(listOfAccount);
	listOfAccount.clear();
}
void Bank::setBank(Bank&& bank)
{
	std::thread t1{ &Bank::moveAccount, this, std::move(bank.listOfAccount) };
	std::thread t2{ &Company::setCompany,
					this,
					std::move(bank.capital),
					std::move(bank.mainOffice),
					std::move(bank.listOfClients),
					std::move(bank.listOfWorkers) };
